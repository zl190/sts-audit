"""
Online Bookstore Order System — Z-level version v2 (decoupled)
v2 adds: ebook category, extra 5% discount when price > 100 + activation code email.
- Strategy pattern  — discount calculation (new EbookDiscount, no changes to existing strategies)
- Observer pattern  — order notifications (new ActivationCodeNotifier, no changes to existing observers)
...
# -- v2: Ebook discount strategy (new class, no changes to existing code) --
...
class DiscountStrategyFactory:
    ...
    _strategies: dict[str, DiscountStrategy] = {
        ...
        "comic":     ComicDiscount(),
        "ebook":     EbookDiscount(),        # v2: register new strategy
    }
    _default = NoDiscount()
...
# -- v2: Activation code notifier (new class, no changes to existing observers) --
...
class OrderService:
    ...
    # -- Business method (v2 requires no changes here! strategies and observers take effect automatically) --
...
def build_service() -> OrderService:
    ...
    service.add_observer(ActivationCodeNotifier())   # v2: register new observer
    return service
